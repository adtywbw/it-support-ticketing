...
- DB restore pipeline (gzip → awk → psql) uses `set -o pipefail` and must run under `bash` via `execFileAsync('bash', ['-c', ...])` in `MaintenanceService.restoreDatabase()`. Using `'sh'` instead of `'bash'` causes `sh: set: Illegal option -o pipefail` and the entire restore fails, leaving the database schema dropped with no data restored.
