...
export default function CurrentSnapshotCards({ current }: CurrentSnapshotCardsProps) {
  const cards = [
    { label: 'Active Tickets', value: current.activeTickets, tone: 'text-navy-950 dark:text-blue-50' },
  ...
  return (
    <section aria-labelledby="current-snapshot-heading" className="space-y-3">
      <h2 id="current-snapshot-heading" className="text-lg font-semibold text-navy-950 dark:text-blue-50">
        Current Snapshot
      </h2>
        ...
        {cards.map((card) => (
          <div key={card.label} className="card">
            <div className="card-body">
              <p className="text-sm font-medium text-navy-500 dark:text-blue-300">{card.label}</p>
