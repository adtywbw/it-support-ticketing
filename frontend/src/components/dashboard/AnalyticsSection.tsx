          ...
          <div className="space-y-2">
            {items.map((item) => (
              <div key={item.key} className="flex items-center gap-3">
                <span className="w-24 text-sm text-gray-600 dark:text-gray-400">{labels[item.key] ?? item.key}</span>
                <div className="h-4 flex-1 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                  <div className={`h-full rounded-full ${colors[item.key] ?? 'bg-gray-400'}`} style={{ width: `${(item.count / total) * 100}%` }} />
                </div>
...
export default function AnalyticsSection({ analytics }: AnalyticsSectionProps) {
  ...
  return (
    <section aria-labelledby="analytics-heading" className="space-y-4">
      <div>
        <h2 id="analytics-heading" className="text-lg font-semibold text-gray-900 dark:text-gray-100">
          Analytics
        </h2>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Ticket Trend</h3>
        </div>
        <div className="card-body">
          {analytics.trend.length === 0 || maxTrend === 0 ? (
            <p className="text-sm text-gray-400 dark:text-gray-500">No ticket activity in this period</p>
          ) : (
            <div className="flex h-36 items-end gap-1">
              {analytics.trend.map((item) => (
                <div key={item.date} className="flex flex-1 flex-col items-center gap-1" title={`${item.date}: ${item.count} tickets`}>
                  ...
                  <span className="origin-left rotate-45 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400">{item.date.slice(5)}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="card">
          <div className="card-body">
            ...
            <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-gray-100">{analytics.slaComplianceRate.toFixed(0)}%</p>
          </div>
        </div>
              ...
              <div className="space-y-2">
                {analytics.topCategories.map((item) => (
                  <div key={item.categoryId} className="flex items-center justify-between text-sm">
                    <span className="truncate text-gray-600 dark:text-gray-400">{item.categoryName}</span>
        ...
        <div className="card">
          ...
          <div className="card-body">
            {analytics.avgResolutionTimeByCategory.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-gray-500">No resolved tickets in this period</p>
            ) : (
              <div className="space-y-2">
                {analytics.avgResolutionTimeByCategory.map((item) => (
                  <div key={item.categoryId} className="flex items-center justify-between gap-3 text-sm">
                    <span className="truncate text-gray-600 dark:text-gray-400">{item.categoryName}</span>
                    <span className="font-medium text-gray-900 dark:text-gray-100">{formatMinutes(item.avgResolutionMinutes)}</span>
                  </div>
                ))}
              </div>
            )}
