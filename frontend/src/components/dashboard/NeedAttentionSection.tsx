...
const priorityClasses: Record<string, string> = {
  ...
  Low: 'bg-blue-50 text-navy-800 dark:bg-navy-800 dark:text-blue-200',
};
...
function TicketRow({ ticket }: { ticket: DashboardTicketSummary }) {
  return (
    <Link
      ...
      className="block rounded-md border border-blue-100 p-3 transition hover:border-primary-300 hover:bg-primary-50 dark:border-navy-800 dark:hover:border-primary-700 dark:hover:bg-primary-950/30"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-navy-950 dark:text-blue-50">
            {ticket.ticketNumber} · {ticket.subject}
          </p>
          <p className="mt-1 text-xs text-navy-500 dark:text-blue-300">
      ...
      <p className="mt-2 text-xs text-navy-500 dark:text-blue-300">
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h3 className="text-base font-semibold text-navy-950 dark:text-blue-50">{title}</h3>
      </div>
      <div className="card-body">
        {tickets.length === 0 ? (
          <p className="text-sm text-navy-500 dark:text-blue-300">{emptyMessage}</p>
        ) : (
...
export default function NeedAttentionSection({ attention }: NeedAttentionSectionProps) {
  return (
    <section aria-labelledby="need-attention-heading" className="space-y-3">
      <h2 id="need-attention-heading" className="text-lg font-semibold text-navy-950 dark:text-blue-50">
        Need Attention
      </h2>
