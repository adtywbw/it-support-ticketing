...
export function splitMinutesForInput(minutes: number): { value: number; unit: SLATimeUnit } {
  if (minutes > 0 && minutes % MINUTES_BY_UNIT.days === 0) {
...
export function isValidSLAWindow(responseTimeMinutes: number, resolutionTimeMinutes: number): boolean {
  return responseTimeMinutes > 0
