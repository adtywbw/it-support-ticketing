...
export function getStatusColor(status: TicketStatus): string {
  const colors: Record<TicketStatus, string> = {
    ...
    Closed: 'bg-blue-50 text-navy-700 dark:bg-navy-800 dark:text-blue-200',
  };
...
export function getPriorityColor(priority: TicketPriority): string {
  const colors: Record<TicketPriority, string> = {
    Low: 'bg-blue-50 text-navy-700 dark:bg-navy-800 dark:text-blue-200',
...
export function getSLAColor(status: string): string {
  ...
  return colors[status] || 'text-navy-600 dark:text-blue-300';
}
