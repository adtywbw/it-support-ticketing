...
describe("useCategories role-aware cache keys", () => {
  ...
  it("should keep category list caches separate by role", async () => {
    ...
    mockGet
      .mockResolvedValueOnce({
        data: { data: [{ id: "cat-1", name: "Minimal" }] },
      })
      .mockResolvedValueOnce({
        data: { data: [{ id: "cat-1", name: "Full", _count: { tickets: 1 } }] },
      });
    ...
    await waitFor(() =>
      expect(first.result.current.data?.[0]?.name).toBe("Minimal"),
    );
    ...
    await waitFor(() =>
      expect(second.result.current.data?.[0]?.name).toBe("Full"),
    );
  ...
  it("should keep category detail caches separate by role", async () => {
    ...
    const first = renderHook(() => useCategory("cat-1"), {
      wrapper: createWrapper(queryClient),
    });
    await waitFor(() =>
      expect(first.result.current.data?.name).toBe("Minimal"),
    );
    ...
    const second = renderHook(() => useCategory("cat-1"), {
      wrapper: createWrapper(queryClient),
    });
