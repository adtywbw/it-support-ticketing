...
const subCategoryId = 'c3d4e5f6-a7b8-4012-9cde-f12345678901';
...
describe('CreateFaqInteractionDto', () => {
  it('accepts RecommendationsShown without faqId', async () => {
    const errors = await validateDto({ sessionId, subCategoryId, eventType: FaqInteractionType.RecommendationsShown });
    expect(errors).toHaveLength(0);
  });

  it('accepts ArticleOpened with faqId', async () => {
    const errors = await validateDto({ sessionId, subCategoryId, eventType: FaqInteractionType.ArticleOpened, faqId });
    expect(errors).toHaveLength(0);
  });

  it('accepts ProblemResolved with faqId', async () => {
    const errors = await validateDto({ sessionId, subCategoryId, eventType: FaqInteractionType.ProblemResolved, faqId });
    expect(errors).toHaveLength(0);
  });
  ...
  it('rejects client-submitted TicketCreated', async () => {
    const errors = await validateDto({ sessionId, subCategoryId, eventType: FaqInteractionType.TicketCreated });
    expect(errors).not.toHaveLength(0);
  });

  it('rejects invalid sessionId', async () => {
    const errors = await validateDto({ sessionId: 'not-a-uuid', subCategoryId, eventType: 'ArticleOpened', faqId });
    expect(errors).not.toHaveLength(0);
  });

  it('rejects invalid faqId', async () => {
    const errors = await validateDto({ sessionId, subCategoryId, eventType: 'ArticleOpened', faqId: 'bad' });
    expect(errors).not.toHaveLength(0);
  });

  it('rejects invalid eventType', async () => {
    const errors = await validateDto({ sessionId, subCategoryId, eventType: 'UnknownEvent' });
    expect(errors).not.toHaveLength(0);
  });

  it('rejects invalid subCategoryId', async () => {
    const errors = await validateDto({ sessionId, subCategoryId: 'bad', eventType: FaqInteractionType.RecommendationsShown });
    expect(errors).not.toHaveLength(0);
  });
});
