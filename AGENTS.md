...
## Auth & Security
- **CSRF protection**: `CsrfGuard` is registered as the first global `APP_GUARD` in `app.module.ts` (before `MaintenanceGuard`). It requires `X-Requested-With: XMLHttpRequest` header on all state-changing requests (POST, PATCH, PUT, DELETE). Safe methods (GET, HEAD, OPTIONS) and exempt paths (`/auth/login`, `/auth/refresh`, `/auth/logout`, `/health`) bypass the check. Additionally, requests with an `Origin` header matching the configured `CORS_ORIGIN` list are trusted without the custom header (browser CORS enforcement ensures credentials-only requests only come from same-origin or explicitly allowed origins). This is the "custom header" pattern — browsers enforce same-origin policy on custom headers, so a cross-origin attacker cannot set `X-Requested-With` via HTML forms or auto-submitting mechanisms.
...
- Frontend filter-panel sync: when a filter panel has local state synced from a parent prop via `useEffect`, use a `justAppliedRef` flag to skip the sync frame immediately after clicking "Apply" — otherwise the parent's new state object triggers the effect and overwrites local edits.
- Backend `??` vs `||` for defaults: use `??` (nullish coalescing) for DTO values where `''` or `0` may be valid inputs. `||` silently substitutes valid falsy values (Session 64 R2).
- Backend shared constants: DTO validation arrays (e.g., sort fields) that are also used in services should be **exported from the DTO** as `as const` arrays and imported by the service — prevents drift between validation and business logic (Session 64 R2).
...
- nginx configs: always add `server_tokens off` to the `http` block. HTTP-only `nginx.conf` must have its own WebSocket rate limit zone (`ws_limit`). All CSP blocks must include `object-src 'none'`. The `frontend/nginx.conf` (used for production self-hosting) must be kept in sync with the main nginx for CSP hardening, cache-control headers, and security headers — it needs separate `/assets/` (immutable), `= /index.html` (no-cache), and `/` (no-cache) location blocks matching the main config layout.
- **nginx `add_header` inheritance**: `add_header` di `server {}` block di-override oleh `location {}` yang punya `add_header` sendiri (nginx behavior). Header yang harus muncul di semua lokasi (e.g., HSTS, CSP) harus diulang di setiap location block. Jangan andalkan server-level inheritance. Fixed di Session 63: HSTS dipindah ke 6 location blocks, helmet HSTS disabled agar tidak double di API path.
...
- **WebP detection** is handled as a special case in `detectMimeFromMagicBytes()` (not in `MIME_SIGNATURES`) because the RIFF header's bytes 4-7 contain the variable file size, which cannot be compared literally. New container formats with variable fields should follow this pattern — add a special-case check after the main signature loop, not a hardcoded-bytes entry.
