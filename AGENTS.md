...
- Repository type safety: avoid `as any` casts in repository methods — use Prisma generics (`Prisma.TicketGetPayload<{}>`, `Prisma.CommentGetPayload<{}>`, etc.) for return types. Methods with dynamic `include`/`select` should cast through `unknown` (e.g., `as unknown as T`). The `as any` pattern is a known bug-incubation layer and should be replaced when encountered. As of Session 34, all production code has been cleared of `as any`; remaining instances exist only in test files (acceptable trade-off for mock flexibility).
